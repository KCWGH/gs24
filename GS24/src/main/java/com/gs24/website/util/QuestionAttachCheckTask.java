...
@Component
@Log4j
public class QuestionAttachCheckTask {
...
//	}
	...
	public void deleteAttachs() {
		...
		List<QuestionAttach> questionAttachList = questionAttachMapper.selectOldList();
		
		// 파일 정보가 없는 경우 종료
		if (questionAttachList.size() == 0) {
			return;
		}
		
		// 파일 정보에서 파일 이름만 추출하여 List<String>으로 변경
		List<String> savedList = questionAttachList.stream() // 데이터 처리 기능 사용을 위한 stream 변경
		...
		File targetDir = Paths.get(attachUploadPath, questionAttachList.get(0).getQuestionAttachPath()).toFile();
	...
	// questionAttach를 전달받아 파일 이름 리턴
	private String toChgName(QuestionAttach questionAttach) {
		return questionAttach.getQuestionAttachChgName();
	}
	
	// attach를 전달받아 파일 이름을 썸네일 파일 이름으로 변경
	private String toThumbnailName(QuestionAttach questionAttach) {
		return "t_" + questionAttach.getQuestionAttachChgName() + "." + questionAttach.getQuestionAttachExtension();
	}
	
	// 확장자로 이미지 파일 확인
	private boolean isImage(QuestionAttach questionAttach) {
		if(questionAttach.getQuestionAttachExtension().equals("jpg") || 
				questionAttach.getQuestionAttachExtension().equals("png") || 
				questionAttach.getQuestionAttachExtension().equals("jpeg") || 
				questionAttach.getQuestionAttachExtension().equals("gif")) {
			return true;
		} else {
			return false;
		}
